...
(defn merge [repo branch-name]
  (let [{h :head bs :branches cs :commits} repo
        [lci fromi toi] (three-way-preapare repo branch-name)
        [lca from to] (select-keys cs [lci fromi toi])
        ; TODO use inds in three-way-merge
...
(defn rebase [repo branch-name]
  (let [ {h :head bs :branches cs :commits} repo
        ...
        ; TODO use some kind of span/break
    ...
    (reduce (fn [r c] (commit r (c :snapshot))) last-comm-repo all-comms)))
...
(defn- three-way-merge
  ...
  (let [[fl ff ft]      (map (fn [c] (get-in c [:snapshot :files]))
                        [lca from to])
...
;(defn- lca
;  [br1 br2 repo]
;  (let [ss1 ((comp sorted-to-set branch-history) repo br1)
;        ss2 ((comp sorted-to-set branch-history) repo br2)]
;    (last (intersection ss1 ss2))))
...
(defn- three-way-preapare
  [repo branch-name]
  (let [{h :head bs :branches} repo
        ->merge (get-in [branch-name :commit-id] bs)
        curr    (get-in [h :commit-id] bs)
        lca     (last-common (branch-history repo branch-name)
                             (branch-history repo h))]
    [lca curr ->merge]))
